# 온습도 센서(DHT11/DHT22) 강의자료 (Markdown)
...
## 1. 학습 목표
- DHT11/DHT22의 차이(정확도, 범위, 속도)를 설명할 수 있다.
- 센서를 ESP32에 배선하고, 라이브러리로 온습도를 읽을 수 있다.
- 측정 주기, 오류 처리(NA/timeout)를 적용할 수 있다.
- 간단한 응용(임계값 경보: LED/버저)을 구현할 수 있다.

---

## 2. 준비물
- ESP32 DevKit 1개
- DHT11 또는 DHT22 센서 (모듈형 권장)
- 점퍼선, 브레드보드
- (옵션) LED + 저항(220Ω), 버저(Active 추천)

---

## 3. DHT11 vs DHT22 비교(슬라이드용)
| 항목 | DHT11 | DHT22(AM2302) |
|---|---|---|
| 습도 범위 | 대체로 20~90%RH | 대체로 0~100%RH |
| 온도 범위 | 대체로 0~50°C | 대체로 -40~80°C |
| 정확도/해상도 | 낮은 편 | 더 좋은 편 |
| 측정 주기 | 느린 편(대기 필요) | 상대적으로 빠름 |
| 추천 | 입문/저가 | 좀 더 신뢰 필요 시 |

> 수업 팁: **DHT11은 값이 “대충”** 나올 수 있어, 그래프/제어용이면 DHT22가 유리.
