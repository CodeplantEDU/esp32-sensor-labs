...
| ESP32 | L9110S | 역할 |
|---|---|---|
| GPIO25 | A-IA | 방향/속도 제어 입력 1 (PWM 가능) |
| GPIO26 | A-IB | 방향/속도 제어 입력 2 (PWM 가능) |
| GND | GND | 공통 접지(필수) |
| (배터리 +) | VCC(VM) | 모터 전원 입력 |
| (배터리 -) | GND | 모터 전원(-) |
...
## A-IA / A-IB 신호에 따른 동작 (표로 넣기용)
> 수업에서 쓰는 “정지”는 보통 **Coast(코스트 정지)** 기준으로 설명하면 깔끔합니다.  
> (두 입력 모두 0이면 모터에 힘을 안 주고 관성으로 멈추는 느낌)

| A-IA | A-IB | 모터 동작(예시) | 설명 |
|---|---|---|---|
| 0(LOW) | 0(LOW) | 정지(코스트) | 힘을 풀고 멈춤 |
| PWM | 0(LOW) | 정방향 | A-IA가 PWM이면 속도 조절됨 |
| 0(LOW) | PWM | 역방향 | A-IB가 PWM이면 속도 조절됨 |
| PWM | PWM | 비추천 | 두 쪽 모두 구동하면 제어가 불안정/의도와 다를 수 있음 (수업에서는 피하기) |

---

## 핀 선택 주의 (GPIO2 / GPIO4 써도 되나?)
- **가능은 하지만 주의가 필요**합니다. GPIO2/4는 보드/부팅 설정(스트랩 핀)과 관련되어, 연결 상태에 따라 **부팅이 불안정**해질 수 있습니다.  
- 수업/실습용으로는 지금처럼 **GPIO25/26 같은 “안전한 핀”**을 쓰는 편이 안정적입니다.
...
- 수업 팁: 실습 2-1로 “내 모터의 최소 출발 PWM”을 먼저 찾고, 그 값보다 조금 크게 LOW를 잡으면 성공률이 높음
